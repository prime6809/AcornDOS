...
.print_info_ifmon
		...
		BNE 	LF265   				; NOMON, exit without displaying info
.print_info
...
.init_fdc_lcat
        ...
        sta     SECCOUNT                
		LDY 	#(LF894-LF87E)          ; Offset of seek command

        lda     #<LF745                 ; Setup INT handler
        sta     INTJMP                  
        lda     #>LF745					
        sta     INTJMP+1                

.LF72F
		LDA 	LF87E,Y         		; Get command byte
        jsr     fdc_send_cmd_drv        ; send it
.LF735
		iny                             ; Increment pointer
		LDA 	LF87E,Y                 ; get next cmd / param
        cmp     #$EA                    ; End ?
		BEQ 	LF743           		; Yep : skip out 
		
		jsr     SEND_PARAM_BYTE         ; Send drive param
		JMP 	LF735					; do next
 
.LF743
		iny                             ; increment pointer & exit
        rts  
...
.LF758
		jsr     copy_for_read           ; Copy read routine into RAM
        ...
        BNE 	LF758                   ; no errors, loop until all read.
.LF765
		rts
...
.START_MOTOR_SELECT
		...
		LDA 	LF7AA,Y					; Get parameter byte
        jsr     SEND_PARAM_BYTE         ; Send it

.LF790
		jsr     TESTREADY               ; Wait for drive ready
		BEQ 	LF790           		
		rts
...
; Copy NMI routine to NMI code space
; ---------------------------------- 
.copy_for_read
		ldy     #$0A                    ; Offset of end of read handler
		BNE 	LF7B4					; Skip if z<>0
.copy_for_write
		ldy     #$12                    ; Offset of end of write handler
.LF7B4
		ldx     #$0B                    ; Byte count
.LF7B6
		lda     NMIREAD,Y               ; Get byte
        sta     TRANSFER-1,X            ; Save in low ram
        dey                             ; decrement counts
        dex                             ; 
		BNE 	LF7B6					; Loop if more
.LF7BF
		rts
...
.LF7DA
		...
		BNE 	LF7BF 					; more retries left, return
...
.get_fdc_result
		lda     IFDCSTATUS               ; Wait until 8271 not busy
        bmi     get_fdc_result           
...
;--------------------------------------------
.read_write_sectors
		jsr     fdc_send_cmd_drv        ; Send command in a to FDC
...
.SEND_PARAM_BYTE
		pha                             ; Save param byte on stack
.LF826
		lda     IFDCSTATUS              ; Get 8271 status
        and     #$20                    ; Mask out bits
		BNE 	LF826                   ; Loop if not ready for param
...
;--------------------------------------------
; note assumes that a contains 0!
;
.set_startaddr
		sta     SECTORNO                
...
; Disk NMI code, copied to NMI space
; ---------------------------------- 
;============================================
...
.NMIREAD
		lda     IFDCDATA                ; Get data from FDC
...
;--------------------------------------------
		...
		JMP 	&00F6                	; Pad out code so address in at &F7/8
...
.LF87E
if (SYS40 = 0)
		EQUB    ICMD_INIT_8271			,$0D				; Init 8271, $0d, 
		EQUB    $03											; step time (ms)
		EQUB    $08											; settle time (ms)
		EQUB    $C0											; index count before unload (msn) head load time (lsn)
		EQUB    $EA 										
else
		EQUB    ICMD_INIT_8271			,$0D				; Init 8271, $0d
		EQUB    $0D											; step time (ms)
		EQUB    $08											; settle time (ms)
		EQUB    $C5											; index count before unload (msn) head load time (lsn)
		EQUB    $EA 
endif
...
.LF894  EQUB    ICMD_SEEK + ICMD_DRIVE0	,$00,$EA             ; Seek to track 0

; NMI handler
; ===========
; Check if pending FDC NMI, otherwise pass on to NMIV
;
.LF897
		PHA                 			; Save a
		lda     IFDCSTATUS              ; Read status regsiter
        and     #$04                    ; Check data ready?
        BEQ 	LF8A8           		; Yep skip

		inc     MEMPTR                  ; Increment src/dest ptr LSB
        bne 	LF8A5                   ; No overflow skip
		
        inc     MEMPTR+1                ; Increment src/dest ptr MSB
.LF8A5
		jmp     TRANSFER                ; Call transfer routine
...
.LF8A8
		...
		JMP (nmiv)         				; Call old NMI
 
.LF8B2
		txa                             ; Save regs on stack
        pha                              
        tya                             
        pha                             
        cld                             
        JSR 	LF8C0                   ; Call int handler
...
.LF8C0
		jmp     (INTJMP)                 ; Jump to int handler
...
.LFC45
		...
		JSR WAIT_NOT_BUSY           ; FC4D 20 5B F2   [r 
		LDY &C2             ; FC50 A4 C2     $B  
.LFC52
...
.LFF0F
		...
		LDX 	#&04					; 4 commands
.LFF30
		JSR 	LF72F                	; Send commands to FDC
		DEX
		BNE 	LFF30
...
.NMIV	EQUW 	LF897           		; FFFA - NMI vector
...
if (ISROM)
  if(SYS40)
    save "sys5-8271-40.rom",start,end,entry
  else
    save "sys5-8271-80.rom",start,end,entry
  endif
endif

;save "sys5-1f-a.rom",start,end,entry